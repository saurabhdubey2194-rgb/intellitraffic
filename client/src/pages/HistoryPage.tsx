...
const ACTION_META: Record<string, { icon: React.ComponentType<{ className?: string }>; color: string; label: string }> = {
  ROUTE_SEARCH: { icon: RouteIcon, color: "bg-sky-500/15 text-sky-300 border-sky-400/40", label: "Route Search" },
  ROUTE_SAVED: { icon: RouteIcon, color: "bg-sky-500/15 text-sky-300 border-sky-400/40", label: "Route Saved" },
  INCIDENT_REPORTED: { icon: AlertTriangle, color: "bg-amber-500/15 text-amber-300 border-amber-400/40", label: "Incident Reported" },
  INCIDENT_STATUS_UPDATE: { icon: AlertTriangle, color: "bg-amber-500/15 text-amber-300 border-amber-400/40", label: "Incident Updated" },
  INCIDENT_REPORT: { icon: AlertTriangle, color: "bg-amber-500/15 text-amber-300 border-amber-400/40", label: "Incident Report" },
  EMERGENCY_TRIP_CREATED: { icon: Siren, color: "bg-red-500/15 text-red-300 border-red-400/40", label: "Trip Started" },
  EMERGENCY_CREATED: { icon: Siren, color: "bg-red-500/15 text-red-300 border-red-400/40", label: "Emergency Created" },
  EMERGENCY_APPROVED: { icon: CheckCircle2, color: "bg-emerald-500/15 text-emerald-300 border-emerald-400/40", label: "Emergency Approved" },
  EMERGENCY_REJECTED: { icon: XCircle, color: "bg-rose-500/15 text-rose-300 border-rose-400/40", label: "Emergency Rejected" },
  CORRIDOR_ACTIVATED: { icon: GitBranch, color: "bg-emerald-500/15 text-emerald-300 border-emerald-400/40", label: "Corridor Activated" },
  CORRIDOR_PROGRESS: { icon: GitBranch, color: "bg-blue-500/15 text-blue-300 border-blue-400/40", label: "Corridor Progress" },
  HOSPITAL_ARRIVAL: { icon: HospitalIcon, color: "bg-emerald-500/15 text-emerald-300 border-emerald-400/40", label: "Hospital Arrival" },
  EMERGENCY_COMPLETED: { icon: CheckCircle2, color: "bg-emerald-500/15 text-emerald-300 border-emerald-400/40", label: "Emergency Completed" },
  USER_REGISTRATION: { icon: Users, color: "bg-violet-500/15 text-violet-300 border-violet-400/40", label: "Registration" },
  AMBULANCE_REGISTRATION: { icon: AmbulanceIcon, color: "bg-violet-500/15 text-violet-300 border-violet-400/40", label: "Ambulance Registration" },
  HOSPITAL_REGISTRATION: { icon: HospitalIcon, color: "bg-violet-500/15 text-violet-300 border-violet-400/40", label: "Hospital Registration" },
  POLICE_REGISTRATION: { icon: Users, color: "bg-violet-500/15 text-violet-300 border-violet-400/40", label: "Police Registration" },
  VERIFICATION_DECISION: { icon: CheckCircle2, color: "bg-emerald-500/15 text-emerald-300 border-emerald-400/40", label: "Verification" },
  VERIFICATION_APPROVED: { icon: CheckCircle2, color: "bg-emerald-500/15 text-emerald-300 border-emerald-400/40", label: "Verification Approved" },
  VERIFICATION_REJECTED: { icon: XCircle, color: "bg-rose-500/15 text-rose-300 border-rose-400/40", label: "Verification Rejected" },
  PROFILE_UPDATE: { icon: Users, color: "bg-slate-500/15 text-slate-300 border-slate-400/40", label: "Profile Update" },
  SIGNAL_SIMULATION: { icon: SignalIcon, color: "bg-cyan-500/15 text-cyan-300 border-cyan-400/40", label: "Signal Simulation" },
  ADMIN_ACTION: { icon: Activity, color: "bg-orange-500/15 text-orange-300 border-orange-400/40", label: "Admin Action" },
  DEMO_EMERGENCY_SIMULATED: { icon: Siren, color: "bg-red-500/15 text-red-300 border-red-400/40", label: "Demo Emergency" },
  LOGIN: { icon: Activity, color: "bg-slate-500/15 text-slate-300 border-slate-400/40", label: "Login" },
};

function actionMeta(actionType: string) {
  return ACTION_META[actionType] ?? { icon: Activity, color: "bg-slate-500/15 text-slate-300 border-slate-400/40", label: actionType };
}

const STATUS_COLOR: Record<string, string> = {
  SUCCESS: "text-emerald-300 bg-emerald-500/15 border-emerald-400/40",
  COMPLETED: "text-emerald-300 bg-emerald-500/15 border-emerald-400/40",
  ACTIVE: "text-blue-300 bg-blue-500/15 border-blue-400/40",
  APPROVED: "text-emerald-300 bg-emerald-500/15 border-emerald-400/40",
  PENDING: "text-amber-300 bg-amber-500/15 border-amber-400/40",
  REJECTED: "text-rose-300 bg-rose-500/15 border-rose-400/40",
  FAILED: "text-rose-300 bg-rose-500/15 border-rose-400/40",
  ARRIVED: "text-cyan-300 bg-cyan-500/15 border-cyan-400/40",
  REPORTED: "text-amber-300 bg-amber-500/15 border-amber-400/40",
};

const ROLE_COLOR: Record<string, string> = {
  public: "text-sky-300",
  ambulance: "text-red-300",
  police: "text-amber-300",
  hospital: "text-emerald-300",
...
function AdminActivityCenter() {
  ...
  const statCards = [
    { label: "Total Activities", value: rows.length, icon: Activity, tone: "text-violet-300" },
    { label: "Today", value: stats.data?.today ?? 0, icon: Calendar, tone: "text-emerald-300" },
    { label: "Police Actions", value: rows.filter(r => r.userRole === "police").length, icon: CheckCircle2, tone: "text-amber-300" },
    { label: "Emergency Events", value: rows.filter(r => r.actionType.startsWith("EMERGENCY") || r.actionType.startsWith("CORRIDOR") || r.actionType === "HOSPITAL_ARRIVAL").length, icon: Siren, tone: "text-red-300" },
  ];
    ...
    <div className="space-y-5">
          ...
          <Card key={s.label} className="border-white/10 bg-white/5">
            <CardContent className="pt-4">
              <div className="flex items-center justify-between">
                ...
                <s.icon className="h-8 w-8 text-white/15" />
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      {stats.data?.today && (
        <div className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-3 text-sm">
          <span className="font-semibold text-emerald-300">Today, IntelliTraffic processed {stats.data.today} actions.</span>{" "}
          <span className="text-slate-300">Real records from the platform's activity engine — not fabricated data.</span>
        </div>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        <Card className="border-white/10 bg-white/5 lg:col-span-3">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-white">Activities by Role</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-white/10 bg-white/5 lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-white">Live Feed</CardTitle>
          </CardHeader>
              ...
              return (
                <div key={r.activityId} className="flex items-start gap-2.5 border-b border-white/5 pb-3 last:border-0">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-xs font-semibold text-white">{r.actionDescription ?? r.actionType}</p>
      ...
      <Card className="border-white/10 bg-white/5">
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex flex-wrap gap-2">
              <Button size="sm" variant="outline" onClick={downloadCSV} className="border-emerald-400/40 text-emerald-300 hover:bg-emerald-500/10">
        ...
        <CardContent>
          {list.isLoading ? (
            ...
            <p className="py-10 text-center text-sm text-slate-400">No activities match your filters.</p>
          ) : (
            <>
              <Table>
                <TableHeader>
                      ...
                      <TableRow key={r.activityId} className="border-white/5">
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                              <p className="truncate text-xs font-medium text-white">{meta.label}</p>
                              <p className="truncate max-w-[280px] text-[11px] text-slate-400">{r.actionDescription}</p>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell className="text-xs">
                          <p className={`font-medium ${ROLE_COLOR[r.userRole ?? ""] ?? "text-slate-300"}`}>{r.userName ?? "System"}</p>
                          <p className="text-[10px] uppercase text-slate-500">{r.userRole ?? "system"}</p>
                        </TableCell>
                        <TableCell>
                        <Badge variant="outline" className={`border-[0.5px] text-[10px] ${STATUS_COLOR[r.status ?? ""] ?? "text-slate-300 border-slate-400/40"}`}>
                          {r.status ?? "—"}
                        </Badge>
                        </TableCell>
                        ...
                        <TableCell className="text-right text-xs text-slate-400">{fmtTime(String(r.createdAt))}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="border-white/10 bg-white/5">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-white">Completed Trips</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {trips.isLoading ? (
              Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-10 bg-white/5" />)
            ) : (trips.data ?? []).length === 0 ? (
              <p className="py-6 text-center text-xs text-slate-400">No completed trips yet.</p>
            ) : (
              (trips.data ?? []).map(t => (
                <div key={t.id ?? t.requestId} className="border-b border-white/5 pb-3 last:border-0">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-white">{t.requestId}</p>
        ...
        <Card className="border-white/10 bg-white/5">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-white">Green Corridors</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {corridors.isLoading ? (
              Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-10 bg-white/5" />)
            ) : (corridors.data ?? []).length === 0 ? (
              <p className="py-6 text-center text-xs text-slate-400">No corridor history yet.</p>
            ) : (
              (corridors.data ?? []).map(c => (
                <div key={c.id ?? c.corridorId} className="border-b border-white/5 pb-3 last:border-0">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-white">{c.corridorId}</p>
        ...
        <Card className="border-white/10 bg-white/5">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-white">Signal Events</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {events.isLoading ? (
              Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-10 bg-white/5" />)
            ) : (events.data ?? []).length === 0 ? (
              <p className="py-6 text-center text-xs text-slate-400">No signal events yet.</p>
            ) : (
              (events.data ?? []).map(e => (
                <div key={e.id} className="border-b border-white/5 pb-3 last:border-0">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-white">Signal #{e.signalId}</p>
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold uppercase ${e.phase === "green" ? "bg-green-500/20 text-green-300" : e.phase === "red" ? "bg-red-500/20 text-red-300" : "bg-amber-500/20 text-amber-300"}`}>
                      {e.phase}
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-400">{(e.previousPhase ?? "-")} → {e.phase} · {(e.optimizedDurationSec ?? e.normalDurationSec) ?? 0}s</p>
    ...
    <div className="space-y-5">
      <div className="grid gap-3 md:grid-cols-3">
        <Link href="/emergency-history">
          <Card className="border-white/10 bg-white/5 transition-colors hover:border-red-400/40">
            <CardContent className="flex items-center gap-3 pt-4">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-red-500/15 text-red-300">
                <Siren className="h-5 w-5" />
              </div>
              <div>
                <p className="text-xs font-semibold text-white">Emergency History</p>
                <p className="text-[11px] text-slate-400">Trips, corridors, arrivals</p>
              </div>
            </CardContent>
          </Card>
        </Link>
        <Link href="/corridor-history">
          <Card className="border-white/10 bg-white/5 transition-colors hover:border-emerald-400/40">
            <CardContent className="flex items-center gap-3 pt-4">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-500/15 text-emerald-300">
                <GitBranch className="h-5 w-5" />
              </div>
              <div>
                <p className="text-xs font-semibold text-white">Corridor History</p>
                <p className="text-[11px] text-slate-400">Green corridors & signals</p>
              </div>
            </CardContent>
          </Card>
        </Link>
        <Link href="/signal-history">
          <Card className="border-white/10 bg-white/5 transition-colors hover:border-cyan-400/40">
            <CardContent className="flex items-center gap-3 pt-4">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-cyan-500/15 text-cyan-300">
                <SignalIcon className="h-5 w-5" />
              </div>
              <div>
                <p className="text-xs font-semibold text-white">Signal History</p>
                <p className="text-[11px] text-slate-400">AI signal optimization log</p>
              </div>
            </CardContent>
          </Card>
        </Link>
      ...
      {incidentRows.length > 0 && (
        <Card className="border-white/10 bg-white/5">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-white">Your Reported Incidents</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2.5">
            {incidentRows.map(inc => (
              <div key={inc.id ?? inc.reportId} className="flex items-center gap-3 border-b border-white/5 pb-2.5 last:border-0">
                ...
                <div className="min-w-0">
                  <p className="truncate text-xs font-medium text-white">{incidentTypeLabel(inc.type)}</p>
                  <p className="truncate text-[11px] text-slate-400">
                    {inc.reportId} · {inc.district} · {inc.createdAt ? new Date(inc.createdAt).toLocaleString("en-IN") : ""}
                  </p>
                </div>
                <Badge variant="outline" className={`ml-auto shrink-0 border-[0.5px] text-[10px] ${STATUS_COLOR[(inc.status ?? "").toUpperCase()] ?? "text-slate-300 border-slate-400/40"}`}>
                  {inc.status ?? "—"}
                </Badge>
              </div>
            ))}
      ...
      <Card className="border-white/10 bg-white/5">
        <CardHeader>
          <CardTitle className="text-sm font-semibold text-white">Activity Timeline</CardTitle>
        </CardHeader>
        <CardContent>
          {list.isLoading ? (
            <div className="space-y-2">
            ...
            <p className="py-10 text-center text-sm text-slate-400">No activity recorded yet. Start exploring — your actions will appear here.</p>
          ) : (
            <div className="space-y-5">
              {Object.entries(grouped).map(([date, rows]) => (
                <div key={date}>
                  <p className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-wide text-slate-400">
                      ...
                      return (
                        <div key={r.activityId} className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.03] px-3 py-2.5">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-xs font-semibold text-white">{r.actionDescription ?? meta.label}</p>
                            <p className="text-[11px] text-slate-400">
                              {r.entityId} · {fmtTime(String(r.createdAt))}
                            </p>
                          </div>
                          <Badge variant="outline" className={`shrink-0 border-[0.5px] text-[10px] ${STATUS_COLOR[r.status] ?? "text-slate-300 border-slate-400/40"}`}>
                            {r.status}
                          </Badge>
                        </div>
                      );
